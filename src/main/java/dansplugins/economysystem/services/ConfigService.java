...
public class ConfigService {
    ...
    /**
     * Copies the bundled usage-reporting block into the server's config.yml when the file
     * on disk has none, so the opt-out is visible where an operator would look for it. The
     * block is normally written by {@link #saveConfigDefaults()}, which runs on a fresh
     * install and on every version change; this covers the remaining case, a config.yml
     * generated by a build that predates usage reporting and carries the same version
     * string. The values come from the defaults Bukkit registered from the jar's config.yml
     * (never new literals), and a file that already has the block is left alone.
     */
    public void ensureUsageReportingBlockOnDisk() {
        if (medievalEconomy.getConfig().isSet("usage-reporting")) {
            return;
        }
        if (medievalEconomy.getConfig().getDefaults() == null) {
            return;
        }
        for (String key : new String[] {USAGE_REPORTING_ENABLED_KEY, USAGE_REPORTING_ENDPOINT_KEY, USAGE_REPORTING_KEY_KEY}) {
            medievalEconomy.getConfig().set(key, medievalEconomy.getConfig().getDefaults().get(key));
        }
        medievalEconomy.saveConfig();
    }
    ...
    // YamlConfiguration, not assumed. ensureUsageReportingBlockOnDisk() copies the block
    // onto disk on enable, so the fall-through is only needed until that has run once.
