...
- A plugin version change is now handled once, not on every startup. The rewritten `config.yml` used to keep the previous `version` string, so the next enable detected the same mismatch, logged the alert again and overwrote `config.yml.old` with the already-rewritten file. The `version` key is now updated to the running plugin; every other value in the file is kept, as before. The alert's wording (and its `Verson` misspelling) now describes that instead of claiming the defaults are reloaded.
- A configuration key added by a later build is now appended to an existing `config.yml`, with its default, the next time the plugin is enabled. The defaults used to be registered only when the file was first generated or the plugin version changed, so a server on the rolling `dev` prerelease — whose successive builds share one version string — never learned a key added since its `config.yml` was written, and every message built from one was shown to the player as the text `null`. The file is written only when a key is actually missing, and the values already in it are kept.
